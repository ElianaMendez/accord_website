-- ACCORD SUPABASE SCHEMA & RLS POLICIES v2.2 (Authoritative RPC Integrity)
-- Target: PostgreSQL / Supabase
-- Description: Strict Row-Level Security isolating direct-client UUID payloads. Result integrity is secured via an authoritative RPC, completely hiding the results table from client DML.

-- 0. RESTRICT DEFAULT PRIVILEGES
...
CREATE TABLE IF NOT EXISTS diagnostic_results (
    ...
    capability_level TEXT NOT NULL CHECK (capability_level IN ('HERO-DEPENDENT', 'FRAGMENTED', 'STRUCTURED', 'INTEGRATED', 'ADAPTIVE')),
...
CREATE INDEX IF NOT EXISTS idx_diagnostic_sessions_owner ON diagnostic_sessions(owner_id);
CREATE INDEX IF NOT EXISTS idx_diagnostic_responses_session ON diagnostic_responses(session_id);
CREATE INDEX IF NOT EXISTS idx_diagnostic_results_session ON diagnostic_results(session_id);
CREATE INDEX IF NOT EXISTS idx_consultation_requests_session ON consultation_requests(session_id);

-- 3. RESULT INTEGRITY RPC (Backend-Authoritative Calculation)
-- The explicit Source of Truth for Diagnostic calculations. The frontend relies on this for accurate completion.
-- Cleansed of BEFORE INSERT triggers as clients are stripped of INSERT privileges to diagnostic_results entirely.

DROP TRIGGER IF EXISTS trg_validate_diagnostic_result ON diagnostic_results;
DROP FUNCTION IF EXISTS validate_diagnostic_result();

CREATE OR REPLACE FUNCTION complete_diagnostic(p_session_id UUID) RETURNS jsonb AS $$
DECLARE
    v_owner_id UUID;
    v_status TEXT;
    v_version TEXT;
...
BEGIN
    ...
    -- Version & Exact Architecture Sequence Verification
    ...
    ELSIF v_version IS NULL THEN
        RAISE EXCEPTION 'Diagnostic completely missing version data.';
    ELSE
        RAISE EXCEPTION 'Unsupported methodology version: %', v_version;
    END IF;

    -- Authoritative Dimensional Math
    ...
    -- Atomic Persistence
    INSERT INTO diagnostic_results(session_id, overall_score, capability_level, primary_vulnerability, secondary_signal, systemic_pattern, recommended_focus)
    VALUES (p_session_id, computed_overall, expected_capability, expected_primary_vuln, expected_secondary_vuln, expected_pattern, expected_focus);

    UPDATE diagnostic_sessions SET status = 'completed', completed_at = v_completed_time WHERE id = p_session_id;

    -- Returning perfectly shaped payload mimicking the normalized front-end structures
...
GRANT SELECT, INSERT ON diagnostic_responses TO authenticated; 
GRANT SELECT ON diagnostic_results TO authenticated; -- Stripped INSERT. Result persistence is restricted entirely to the RPC.
GRANT SELECT, INSERT ON consultation_requests TO authenticated;

-- Service Role retains full capability Server-Side
...
-- COMPANIES
CREATE POLICY "Auth owner can select own companies" ON companies FOR SELECT TO authenticated USING (auth.uid() = owner_id);
CREATE POLICY "Auth owner can insert own companies" ON companies FOR INSERT TO authenticated WITH CHECK (auth.uid() = owner_id);
CREATE POLICY "Auth owner can update own companies" ON companies FOR UPDATE TO authenticated USING (auth.uid() = owner_id) WITH CHECK (auth.uid() = owner_id);

-- DIAGNOSTIC_SESSIONS (with strict foreign company ownership resolution check)
CREATE POLICY "Auth owner can select own sessions" ON diagnostic_sessions FOR SELECT TO authenticated USING (auth.uid() = owner_id);
CREATE POLICY "Auth owner can insert own sessions" ON diagnostic_sessions FOR INSERT TO authenticated 
WITH CHECK (
    auth.uid() = owner_id AND 
    (company_id IS NULL OR EXISTS (SELECT 1 FROM companies WHERE id = company_id AND owner_id = auth.uid()))
);
CREATE POLICY "Auth owner can update own sessions" ON diagnostic_sessions FOR UPDATE TO authenticated 
...
-- DIAGNOSTIC_RESPONSES (Dropped UPDATE grant safely for immutability once inserted)
CREATE POLICY "Auth owner can select own responses" ON diagnostic_responses FOR SELECT TO authenticated
USING (EXISTS (SELECT 1 FROM diagnostic_sessions s WHERE s.id = diagnostic_responses.session_id AND s.owner_id = auth.uid()));

CREATE POLICY "Auth owner can insert own responses" ON diagnostic_responses FOR INSERT TO authenticated
WITH CHECK (EXISTS (SELECT 1 FROM diagnostic_sessions s WHERE s.id = session_id AND s.owner_id = auth.uid()));

-- DIAGNOSTIC_RESULTS (Stripped INSERT policy. Pure read-only context.)
CREATE POLICY "Auth owner can select own results" ON diagnostic_results FOR SELECT TO authenticated
USING (EXISTS (SELECT 1 FROM diagnostic_sessions s WHERE s.id = diagnostic_results.session_id AND s.owner_id = auth.uid()));

-- CONSULTATION_REQUESTS
CREATE POLICY "Auth owner can view own consultation" ON consultation_requests FOR SELECT TO authenticated
USING (EXISTS (SELECT 1 FROM diagnostic_sessions s WHERE s.id = consultation_requests.session_id AND s.owner_id = auth.uid()));

CREATE POLICY "Auth owner can insert own consultation" ON consultation_requests FOR INSERT TO authenticated
WITH CHECK (EXISTS (SELECT 1 FROM diagnostic_sessions s WHERE s.id = session_id AND s.owner_id = auth.uid()));
